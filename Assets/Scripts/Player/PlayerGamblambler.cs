...
namespace Player
{
    public class PlayerGamblambler : PlayerCharacter
    {
        public virtual void Attack()
        {
            ...
            {
                case 0:
                    damageOut *= (100 - enduranceStat) / 100;
                    LogManager.instance.InstantiateDamageLog(characterName, "itself", damageOut);
                    TakeDamage(damageOut);
                    break;
                case 1:
                    damageOut *= (100 - EnemyManager.instance.targetEnemy.enduranceStat) / 100;
        ...
        {
            var healOut = enduranceStat + EnemyManager.instance.targetEnemy.enduranceStat;
            var targetRoll = Random.Range(0, 2);
            switch (targetRoll)
            {
                case 0:
